...
**文本输入框**:
```javascript
find: "真诚分享经验或资讯 textbox textarea"
```

**输入方法**:
1. 点击文本框（这是一个 DIV 元素，不是标准 input）
2. 使用 `type` 动作输入标题内容

**示例**:
```javascript
click(ref_49)  // 点击文本框
type("测试标题 - 小红书发布测试")  // 输入标题
```
...
**重要**: 点击"下一步"后，如果只有一张图片，会回到"写文字"编辑页面继续添加第二张图片。
...
**新编辑器特征**:
- 顶部标签: "正文页-1"
- 占位符: "在这里输入正文，描述事情发展或者分观点陈述"

**正文输入框**:
```javascript
find: "在这里输入正文 textbox textarea"
```

**输入方法**:
```javascript
click(ref_209)  // 点击正文输入框
type("这是正文内容...")  // 输入正文
```

**生成图片**:
再次点击"生成图片"按钮，进入模板选择页面。
...
element_type: textbox (text input)
max_length: 20字符
```

**输入方法**:
```javascript
form_input(ref_227, "小红书自动发布测试")
```

**正文文本框**:
```javascript
find: "测试标题 - 小红书发布测试 textbox textarea"
element_type: textarea
```

**特殊说明**:
- 正文区域会自动填充第一张图片的内容（标题）
- **必须填充完整的正文内容**（不是摘要或部分内容）
- 正文内容包括：完整正文 + 两个换行 + 标签
- 字符计数显示在右侧
...
```javascript
click(ref_230)  // 点击正文文本框
key("cmd+ArrowRight")  // 跳到文本末尾
type("\n\n#小红书发布 #自动化工具 #效率提升")  // 换两行后添加标签
```
